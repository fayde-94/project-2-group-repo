document
      ...
      const anyKeyPressToLoadMainPage=()=>{
        if (!document.getElementById('mainPage').innerHTML){
          $("#mainPage").load("navMainCarousel.html",function(responseTxt, statusTxt, xhr){showSlides(1);});
                 }
      }
      const enterKeyPress=()=>{
       ...
       const selectedElement=document.getElementsByClassName("verticalCardActive");
        if(selectedElement.length > 0){
          if(selectedElement[0].classList.contains('smartLightOFF')){onEnterOfSmartSwitch(selectedElement,0);}
